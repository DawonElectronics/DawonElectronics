...
namespace DAWON_UV_INVENTORY_PROTO.Views
{
    ...
    /// </summary>
    public partial class EditUserWindowFinish : ChromelessWindow
    {
        ViewUvWorkorderDone record;
        List<TbUsers> _users = new List<TbUsers>();
        GridRecordContextMenuInfo objsender = new GridRecordContextMenuInfo();
        public EditUserWindowFinish(object obj)
        {
            InitializeComponent();

            record = (ViewUvWorkorderDone)(obj as GridRecordContextMenuInfo).Record;
            ...
            using (var db = new Db_Uv_InventoryContext())
            {
                ...
                if (result != null)
                {
                    if (record.IsDone == true)
                    {
                        ...
                        MainWindow._mainwindowViewModel.SelectedGridFinish.TrackinUsername = inuser;
                        MainWindow._mainwindowViewModel.SelectedGridFinish.TrackoutUsername = outuser;
